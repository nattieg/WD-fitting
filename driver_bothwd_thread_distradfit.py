...
#this version will not assume the WD core composition before the fit. 
#Instead, we will fit the normalization = r^2/d^2, then use the distance
#to find the corresponding radius. The radius and the log g will give the mass
...
def wdparams2(waves, ps):
    modg1, modT1, rad1, modg2, modT2, rad2, dist = ps
    wave1, wave2 = waves
    fitflux1 = wdparams(wave1, (modg1, modT1, rad1, dist), interpfunc1)
    fitflux2 = wdparams(wave2, (modg2, modT2, rad2, dist), interpfunc2)
...
minR = np.min(Ro) #* Rsun
maxR = np.max(Ro_He) #* Rsun
#minnorm = minR**2.0 / (distmax*parsec)**2.0
#maxnorm = maxR**2.0 / (distmin*parsec)**2.0

#define the function
func = wdparams2
#make guesses at parameters : 

#norm1guess = (0.02*Rsun)**2.0 / (dobs*parsec)**2.0
#print("This is the norm1 guess:")
#print(norm1guess)

#norm2guess = (0.018*Rsun)**2.0 / (dobs*parsec)**2.0
#print("This is the norm2 guess:")
#print(norm2guess)

#log g1, T1, R1, logg2, T2, R2, dist
params = (7.5, 15600., 0.02, 7.6, 16600., 0.018, dobs) 
eparams = (1.0, 1000., 0.002, 1.0, 1000., 0.002, dobserr) 
#distance fit in units of pc 
#give the parameter names for plotting (can use latex symbols)
params_name = [r'log$_{10}$(g$_{5379}$)',r'Temp$_{5379}$(K)', r'$R_{5379}$',r'log$_{10}$(g$_{4540}$)',r'Temp$_{4540}$(K)',r'$R_{4540}$',r'dist (pc)']
#provide bounds on the parameters used to draw the initial guesses and walkers and possibly to limit the priors
bnds = ( (6., 9.), (14000., 20000.), (minR, maxR), (6., 9.), (14000., 20000.), (minR, maxR), (distmin, distmax))
...
f2.savefig('initial_guess_fit_wd_distrad.png')
...
#for plots
doplots = True
f1name = "walkers_distrad.png"
f2name = "fit_MCMC_distrad.png"
f3name = "chains_distrad.png"
f4name = "posterior_distrad.png"
#f5name = "fit_leastsq_wd2.png"
#output 
results = []
print_posterior = True
posterior_file = "posterior_distrad.txt"
