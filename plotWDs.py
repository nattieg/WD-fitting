...
p1, p2, p3, p4, p5 = np.loadtxt('posterior_wd2.txt', unpack=True)
...
f1name = "WD1spectrum_hst.pdf"
f2name = "WD2spectrum_hst.pdf"
...
for i in xrange(0, nsize):
	index = np.random.randint(0, len(p1), size=3)
	#print(index)
#	postflux1, postflux2 = wdparams2((modwave1,modwave2), (p1[np.int(index[0])],p2[np.int(index[1])],p3[np.int(index[2])],p4[np.int(index[3])],p5[np.int(index[4])]))
	postflux1, postflux2 = wdparams2((modwave1,modwave2), (p1[np.int(index[0])],p2[np.int(index[0])],p3[np.int(index[1])],p4[np.int(index[1])],p5[np.int(index[2])]))
	#print(postvalues[i])
...
ax1.errorbar(wave1, ydata1, yerr=sigma1, fmt='.', zorder=2, color="#3b7cc8", capsize=0)
ax2.errorbar(wave2, ydata2, yerr=sigma2, fmt='.', zorder=2, color='#3b7cc8', capsize=0)

pout = [(7.6273545051010689, 0.045767302410375521, 0.053557987774747851), (15897.289586570994, 196.11846712461556, 289.21957607769582), (7.777726201853234, 0.086444814796751146, 0.083675204668610448), (16808.808172330584, 296.92408019143113, 264.57081718816335), (1804.8049466850844, 29.336839388689896, 52.16601436536871)]
